...
function initial()
{
	...
	$sql_movimientos = _query("SELECT * FROM mov_caja WHERE id_sucursal = '$id_sucursal' AND fecha BETWEEN '$fecha_1' AND '$fecha_2' ORDER BY id_movimiento DESC");
...
function search()
{
	...
	$sql_movimientos = _query("SELECT * FROM mov_caja WHERE id_sucursal = '$id_sucursal' ORDER BY id_movimiento DESC");
	$cuenta = _num_rows($sql_movimientos);
	$lista = "";
	if ($cuenta>0){
	$i = 1;
