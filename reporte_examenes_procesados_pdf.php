...
$sql4="SELECT e.nombre_examen , p.nombre as p1, p.apellido as p2, ep.fecha_realizado , ep.hora_realizado, em.nombre as n1, em.apellido  as n2
From examen_paciente as ep
LEFT JOIN paciente as p ON(p.id_paciente=ep.id_paciente)
LEFT JOIN examen as e ON(e.id_examen=ep.id_examen)
LEFT JOIN empleado as em ON(em.id_empleado=ep.id_empleado)
WHERE ep.id_examen>0 AND ep.estado_realizado='Hecho' AND ep.estado_impresion='Pendiente' and ep.id_sucursal='$id_sucursal' AND ep.examen_paciente_nulo= 0 AND ep.fecha_examen BETWEEN '$antes' AND '$hoy'";
...
$pdf->SetXY($set_x, $set_y);
$pdf->Cell(215,5,utf8_decode("REPORTE DE EXAMENES PROCESADOS"),0,1,'C');
$pdf->SetFont('courier','',10);
$pdf->SetXY($set_x, $set_y+5);
...
if(_num_rows($result3)>0)
{
  ...
  while($row3 = _fetch_array($result3))
  {
    ...
    $fecha_examen = $row3["fecha_realizado"];
    $hora_examen = $row3["hora_realizado"];
