...
$sqlcobro=_query("SELECT mu.muestra,c.id_cobro,CONCAT(p.nombre,' ',p.apellido) as paciente,p.sexo,p.fecha_nacimiento,ep.id_examen_paciente,CONCAT(dr.nombre,' ',dr.apellido) as doctor,pr.nombre as procedencia,ep.fecha_realizado,e.nombre_examen
...
WHERE ep.id_cobro='$id_cobro' AND ep.estado_realizado='Hecho' GROUP BY ep.id_examen_paciente ORDER BY e.prioridad ASC, CHARACTER_LENGTH(ep.resultados) ASC ");
...
$edad= $rowcobro["fecha_nacimiento"];
$sexo = $rowcobro["sexo"];
...
$hmex=_query("SELECT COUNT(ep.id_examen_paciente) as cantidad
FROM cobro as c
JOIN examen_paciente as ep ON ep.id_cobro=c.id_cobro
JOIN examen as e ON e.id_examen=ep.id_examen
LEFT JOIN examen_perfil epe ON epe.id_examen=e.id_examen
LEFT JOIN perfil as pe ON pe.id_perfil=epe.id_perfil
WHERE c.id_cobro='$id_cobro' AND ep.estado_realizado='Hecho'  GROUP BY c.id_cobro ");
$rowex = _fetch_array($hmex);
$CE = $rowex["cantidad"];
...
LEFT JOIN examen_perfil epe ON epe.id_examen=e.id_examen
LEFT JOIN perfil as pe ON pe.id_perfil=epe.id_perfil
WHERE c.id_cobro='$id_cobro' AND ep.estado_realizado='Hecho' GROUP BY cat.id_categoria ORDER BY cat.id_categoria ASC,e.prioridad ASC, CHARACTER_LENGTH(ep.resultados) ASC");
while($rowcat = _fetch_array($sqlcat)){
    ...
    $sqlcobro1=_query("SELECT c.id_cobro,e.id_examen,e.nombre_examen,ep.resultados,mu.muestra
    FROM cobro as c JOIN detalle_cobro as dc ON dc.id_cobro=c.id_cobro
    JOIN examen_paciente as ep ON ep.id_cobro=c.id_cobro
    JOIN examen as e ON e.id_examen=ep.id_examen
    LEFT JOIN examen_perfil epe ON epe.id_examen=e.id_examen
    ...
    WHERE c.id_cobro='$id_cobro' AND ep.estado_realizado='Hecho' AND cat.id_categoria='$id_categoria' GROUP BY ep.id_examen_paciente ORDER BY e.prioridad ASC, CHARACTER_LENGTH(ep.resultados) ASC  ");
    ...
    while($rowcobro1 = _fetch_array($sqlcobro1)){
        ...
        if($conteoparametros==1){
          if($primerparametro==1){
              ...
              if($CE==1){
                  ...
                  $pdf->SetFillColor(178, 207, 255);
                  $pdf->Cell(215,5,utf8_decode(str_replace("IGM","IgM",str_replace("IGG","IgG",$nombre_examen))),0,1,'C',1);
                  $pdf->SetFillColor(255,255,255);
          ...
          for($i=0; $i<(count($formulario)-1); $i++){
              ...
              if(strpos($nombre_examen,'TSH ICROMAC')!==false){
                $pdf->MultiCell(50,3,"TSH",0,'L');
              }else{
                $pdf->MultiCell(50,3,utf8_decode(str_replace("IGM","IgM",str_replace("IGG","IgG",Mayu($nombre_examen)))),0,'L');
              }
              $pdf->SetXY($set_x+60, $set_y+$mm);
              if(strpos($nombre_examen,'FROTIS SANGRE PERIFERICA')!==false || strpos($nombre_examen,"CULTIVO") !==false || strpos($nombre_examen,'FROTIS SANGRE PERIFERICA (NN)') !==false){
                $pdf->MultiCell(150,3,utf8_decode(str_replace('@','+',$campos_valores[1])),0,'L');
              }else{
              ...
              if(Mayu($nombre_examen)=='ERITROSEDIMENTACION'){
                $pdf->SetXY($set_x+102, $set_y+$mm);
                $pdf->Cell(20,8,utf8_decode("EDAD"),1,1,'C');
                $pdf->SetXY($set_x+122, $set_y+$mm);
                $pdf->Cell(19,8,utf8_decode("HOMBRE"),1,1,'C');
                $pdf->SetXY($set_x+141, $set_y+$mm);
                $pdf->Cell(19,8,utf8_decode("MUJER"),1,1,'C');
                $mm+=8;
                $j++;
                for($p=0; $p<6; $p++){
                  $minoria=explode("<", $division[$p]);
                  $pdf->SetXY($set_x+102, $set_y+$mm);
                  $pdf->Cell(20,8,utf8_decode($minoria[0]),1,1,'C');
                  $pdf->SetXY($set_x+122, $set_y+$mm);
                  $pdf->Cell(19,8,utf8_decode("<".$minoria[1]),1,1,'C');
                  $pdf->SetXY($set_x+141, $set_y+$mm);
                  $pdf->Cell(19,8,utf8_decode("<".$minoria[2]),1,1,'C');
                  $mm+=8;
                  $j++;
                  //$final=true;
                  $pdf->SetFont('latin','',9);
              ...
              else if(strpos($nombre_examen,'ERITROSEDIMENTACION 1 HORA')!==false){
        ...
        if($conteoparametros>1){
            ...
            if(trim($nombre_examen) == "HEMOGRAMA"){
                ...
                else if($n_cat==0 AND $pagenew==true AND $acti==0){
                    if($j>=$salto){
                        $pdf->AddPage('P','Letter');
                        //DATOS DEL PACIENTE
                        $set_y = 45;
                ...
                /*else if($pagenew==true AND $acti==0 AND $pagenew){
                    $pdf->SetFont('latin','',11);
                    $pdf->SetXY($set_x-10, $set_y-5+$mm);
                    $pdf->SetFillColor(178, 207, 255);
                    $pdf->Cell(215,5,utf8_decode($n_categoria),0,1,'C',1);
                    ...
                    $pdf->Cell(50,5,utf8_decode("RESULTADO"),0,1,'L');
                    $pdf->SetXY(110, $set_y+$mm);
                    $pdf->Cell(205,5,utf8_decode("VALORES DE REFERENCIA"),0,1,'L');$j+=2;
                    $pdf->SetXY(170, $set_y+$mm);
                    $pdf->Cell(50,5,utf8_decode("UNIDADES"),0,1,'L');
                    $mm+=8;
                }*/
            ...
            for($i=0; $i<(count($formulario)-1); $i++){
                ...
                if($campos_valores[4]=='s'){
                    $pdf->SetFont('latin','',9);
                    $pdf->SetXY($set_x, $set_y+$mm);
                    $pdf->Cell(55,5,utf8_decode(str_replace("IGM","IgM",str_replace("IGG","IgG",Mayu($campos_valores[0])))),1,1,'C');
                    $mm+=8;
                    $restante-=5;
                    $j++;
                }
                    ...
                    $pdf->SetFont('latin','',9);
                    $pdf->SetXY($set_x, $set_y+$mm);
                    $pdf->MultiCell(55,3,utf8_decode(str_replace("IGM","IgM",str_replace("IGG","IgG",Mayu(utf8_decode($campos_valores[0]))))),0,'L');
                    $pdf->SetXY($set_x+60, $set_y+$mm);
                    ...
                    $pdf->Cell(3,3,$dats_print[1].",".$dats_print[2],0,1,'L');
                        ...
                        if(strpos($nombre_examen,'FROTIS SANGRE PERIFERICA')!==false || strpos($nombre_examen,'FROTIS SANGRE PERIFERICA (NN)')!==false){
                            $pdf->MultiCell(150,3,utf8_decode(str_replace('@','+',$campos_valores[1])),0,'L');
                        }else{
                            $pdf->Cell(30,3,utf8_decode(str_replace('@','+',$campos_valores[1])),0,1,'L');
                        }
                        //$pdf->Cell(30,3,utf8_decode(str_replace('@','+',$campos_valores[1])),0,1,'L');
                    }
                    }
                        ...
                        if(trim($campos_valores[3]) =="4,000,000 - 5,500,000"){
                            ...
                            if(count($division)>1){
                                for($k=0; $k<=(count($division)-1); $k++){
                                    ...
                                    if(strpos(Mayu($campos_valores[0]),'T.G.O.')!==false){
                                      if($sexo=="F"){
                                        if(strpos($division[$k],'Mujer')!==false){
                                          $mm-=8;
                                          $j--;
                                          $pdf->SetXY($set_x+102, $set_y+$mm);
                                          $pdf->Cell(30,3,utf8_decode(trim($division[$k])),0,1,'L');
                                        }
                                        /*if(strpos($division[$k],'Hombre')!==false){
                                          //$pdf->SetXY($set_x+102, $set_y+$mm);
                                          //$pdf->Cell(30,3,utf8_decode(trim($division[$k])),0,1,'L');
                                        }*/
                                      }else if($sexo=="M"){
                                    ...
                                    else if(strpos(Mayu($campos_valores[0]),'T.G.P.')!==false){
                                      if($sexo=="F"){
                                        if(strpos($division[$k],'Mujer')!==false){
                                          $mm-=8;
                                          $j--;
                                    ...
                                    /*else if(strpos(Mayu($campos_valores[0]),'FROTIS SANGRE PERIFERICA (NN)')!==false){
                                        $pdf->SetXY($set_x+102, $set_y+$mm);
                                        $pdf->MultiCell(30,3,utf8_decode(trim($division[$k])),0,'L');
                                    }*/
                                    else{
                                      $pdf->SetXY($set_x+102, $set_y+$mm);
                                      $pdf->Cell(30,3,utf8_decode(trim($division[$k])),0,1,'L');
