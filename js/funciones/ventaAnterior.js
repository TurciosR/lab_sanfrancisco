	  ...
	  $('#inventable tr #doctor').select2({
	      ...
	      language: {
	        noResults: function() {
	          ...
	          return modalDoctor;
	          $('#inventable tr #doctor').select2('close');
	        }
	      }
	    });
...
function cargar_ref()
{
    ...
    success: function(datax) {
			...
			{
				a.empty().trigger('change');
				b.empty().trigger('change');
				c.empty().trigger('change');

				$("#vendedor").html(datax.select_vendedor);
				a.trigger('change');
...
/*<input type='text'  class='form-control decimal' readOnly id='precio_venta' name='precio_venta' value='" + data.preciop + "'>*/
      ...
      tr_add += "<tr  class='row100 head' id='" + filas + "'>";
			...
			tr_add += "<td class='cell100 column20 text-success descp' id='desc'>" + data.descripcionp + "</td>";
			...
			tr_add += "<td class='cell100 column12 text-success hor'> " + data.horas + "</td>";
			tr_add += "<td class='cell100 column8 text-success' id='precio_venta'><input type='hidden'  id='precio_venta_inicial' name='precio_venta_inicial' value='" + data.preciop + "'><input type='hidden'  id='precio_sin_iva' name='precio_sin_iva' value='" + preciop_s_iva+ "'>" + data.preciop + "</td>";
			tr_add += cantidades;
			//tr_add += "<td  class='cell100 column10 text-success'><input type='text'  class='form-control decimal' readOnly id='precio_venta' name='precio_venta' value='" + subtotal + "'></td>";

			if(tipo_impresion=="CCF")
      {
        tr_add += "<td  hidden class='ccell100 column7'>" + "<input type='hidden'  id='subtotal_fin' name='subtotal_fin' value='"+preciop_s_iva+"'>" + "<input type='hidden'  class='decimal form-control' id='subtotal_mostrar' name='subtotal_mostrar'  value='" + preciop_s_iva.toFixed(2) + "'style='width:55px;'readOnly></td>";

      }
      else
      {
        tr_add += "<td hidden class='ccell100 column7'>" + "<input type='hidden'  id='subtotal_fin' name='subtotal_fin' value='"+subtotal+"'>" + "<input type='hidden'  class='decimal txt_box' id='subtotal_mostrar' name='subtotal_mostrar'  value='" + subt_mostrar + "'style='width:55px;'readOnly></td>";
...
{
	var tr =$(this).parents('tr');
	tr.find(".fecha").datepicker('open');
});
$(document).on('datepicker:close', '.fecha', function(evt){
	var tr = $(this).parents('tr');
	if(evt.keyCode == 13)
	{
		tr.find(".hora").focus();
...
$(document).on('datepicker:close', '.hora', function(){
	var tr = $(this).parents('tr');
