...
function send() {
  ...
  var num_doc_pago = $('#num_doc_pago').val();
  var tipo = $('#tipo').val();
  $("#monto").val("");
  var cuenta_bank=$("#cuenta_bank").val();

  var dataString = 'process=abonar'+'&id_factura='+id_factura+"&monto="+monto+"&tipo_doc="+tipo_doc+"&num_doc="+num_doc+"&tipo_pago="+tipo_pago+"&num_doc_pago="+num_doc_pago+"&cuenta_bank="+cuenta_bank+"&tipo="+tipo;
  $.ajax({
    type: "POST",
...
$(document).on('click', '#abonar', function(event) {
  ...
  if (monto != undefined && monto != 0 && monto != '') {
    ...
    else
    {
      ...
      var num_doc_pago=$("#num_doc_pago").val();
      if (num_doc_pago != undefined && num_doc_pago != 0 && num_doc_pago != '') {
        if($("#tipo_pago").val()=="Transferencia")
        {
          var cuenta_bank=$("#cuenta_bank").val();
          if (cuenta_bank != undefined && cuenta_bank != 0 && cuenta_bank != '')
          {
            send();
          }
          else
          {
            display_notify('Error', 'No ha seleccionado una cuenta de banco');
        ...
        display_notify('Error', 'No ha ingresado un numero de documento para el tipo de pago');
