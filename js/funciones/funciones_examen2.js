...
$(function() {
  ...
  //funcio para anadir precios a la tabla
  $(document).on("click", "#ingresar_precio", function() {
    var precio=$("#a_precios").val();
    if(precio==""){
      display_notify("Error", "Por favor agrege un precio");
    }
    else
    {
    var app = "<tr><td class='precio'>"+precio+"</td><td class='text-center'><a class=' lndelete' type='button' name='button'> <span class='fa fa-trash'></span> </a></td></tr>";
    $("#lprecio").append(app);
    }

  })
  $(document).on('keypress', '#a_precios', function(event) {
  var precio=$(this).val();
    if (event.key == 'Enter') {
      if(precio==""){
        display_notify("Error", "Por favor agrege un precio");
...
$(document).on("click", "#sig", function(event) {
  $('#formulario').validate({
    rules: {
      id_categoria: {
        required: true,
    ...
    if (error) {
      ...
      if (error2) {
        ...
        if(error3){
          ...
          var codigo_examen = $('#codigo_examen').val();
          //Get the value from form if edit or insert
          var process = $('#process').val();
          if (process == 'insert') {
            var id_examen = 0;
            var urlprocess = 'agregar_examen.php';
            var dataString = 'process=' + process + '&id_categoria=' + id_categoria + '&nombre_examen=' + nombre + '&codigo_examen=' + codigo_examen + '&formulario=' + formulario + '&insumos=' + insumos+ '&precios=' + datos_precio;
          }
          if (process == 'edited') {
            var id_examen = $('#id_examen').val();
            var urlprocess = 'editar_examen.php';
            var dataString = 'process=' + process + '&id_categoria=' + id_categoria + '&nombre_examen=' + nombre + '&codigo_examen=' + codigo_examen + '&formulario=' + formulario + '&id_examen=' + id_examen + '&insumos=' + insumos + '&precios=' + datos_precio;
          }
          $.ajax({
            type: 'POST',
            url: urlprocess,
