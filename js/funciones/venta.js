var urlprocess='';
var id_cobro_facturacion=0;
var cambio_enviado = 0;
var monto_enviado = 0;
...
function senddata() {
  ...
  var edad_paciente = $("#naci").val();
	var fecha_nacimiento = $("#fecha_nacimiento").val();
	var verificar_edad_fecha = $("#check_nacimiento").val();
  var telefono = $("#telefono").val();


	var error_examen_covid = 0;
	...
	$("#inventable tr").each(function(index)
	{
		...
		if(id_exa == 698 || id_exa == 699 || id_exa == 700){
			if(pasaporte == ""){
				error_examen_covid++;
			}
		}
  ...
  dataString += '&sexo_paciente=' + sexo_paciente;
  dataString += '&edad_paciente=' + edad_paciente;
	dataString += '&fecha_nacimiento=' + fecha_nacimiento;
	dataString += '&verificar_edad_fecha=' + verificar_edad_fecha;
  dataString += '&telefono=' + telefono;
	...
	dataString += '&pasaporte=' + pasaporte;
	dataString += '&dui=' + dui;

	var sel_vendedor = 1;
	if(error_examen_covid == 1){
		msg = 'Ingrese el Pasaporte, es necesario para la PRUEBA DE ANTIGENO COVID-19!';
		sel_vendedor = 0;
	}
	else{
		...
		if(verificar_edad_fecha==3){
			if(pacientee=="" && (paciente=="" || sexo_paciente=="" || fecha_nacimiento=="")){
			console.log(verificar_edad_fecha);
			msg = 'Completar los datos nombre, edad y sexo del paciente!';
			sel_vendedor = 0;
			}

		}else{
			if(pacientee=="" && (paciente=="" || sexo_paciente=="" || edad_paciente=="")){
			console.log(verificar_edad_fecha);
			msg = 'Completar los datos nombre,edad y sexo del paciente!';
			sel_vendedor = 0;
			}
	...
	if (sel_vendedor == 1 && error_examen_covid == 0) {
		...
		$.ajax({
			...
			success: function(datax) {
				if (datax.typeinfo == "Success")
				{
					...
					id_cobro_facturacion = datax.id_cobro;
				}
				else {
...
$(document).on("keyup","#efectivov",function(evt){
	...
	else
	{
		if(parseFloat($("#cambiov").val()) >=0)
		{
			...
			//setTimeout(function(){ reload1(); }, 1500);
		}
		else {
...
function factura_imprimir(){
	$.ajax({
		url: 'venta.php',
		type: 'POST',
		data: 'process=imprimir_ticket_cobro&id_cobro='+id_cobro_facturacion+"&monto_enviado="+monto_enviado+"&cambio_enviado="+cambio_enviado,
		dataType: 'JSON',
		async: true,
		...
		cache: false,
		success: function(data) {
			var sist_ope = data.sist_ope;
			var dir_print = data.dir_print;
			var shared_printer_pos = data.shared_printer_pos;
			var datos = data.datosvale;
			datos = datos.replace(new RegExp("&nbsp;", "g"), "");
			if (sist_ope == 'win') {
				$.post("http://"+dir_print+"printvalewin1ti.php", {
					datosvale: data.datosvale,
				...
				//  footers:footers,
					...
					$.post("http://"+dir_print+"printvale1ti.php", {
						datosvale: data.datosvale,
						id_cobro:data.id_cobro
					});
				}
...
function total_efectivov(){
	...
	var cambio=round(cambio, 2);
	var	cambio_mostrar=cambio.toFixed(2);
	cambio_enviado = cambio_mostrar;
	monto_enviado = efectivo;
...
$(document).on("click", "#btnAddDoctor", function() {
	var nombres = $(".modal-body #nombre").val();
	var apellido = $(".modal-body #apellido").val();
	...
	if (nombres!="") {
		...
		if (apellido!="") {
				...
				if(especialidad!=""){
					...
					display_notify('Error','Por favor seleccione especialidad!');
				}
		}else {
			display_notify('Error','Por favor ingrese apellido!');
		}

	}else {
		display_notify('Error','Por favor ingrese nombre!');
...
$(document).on("click", "#btnAddProcedencia", function(event) {
	...
	var nombres = $(".modal-body #nombre").val();
	var apellido = $(".modal-body #descripcion").val();
	var espe = $(".modal-body #telefono").val();
	if (nombres!="") {
		...
		if(apellido!="") {
				...
				if(espe !=""){
					...
					display_notify('Error','Por favor ingrese telefono!');
				}
		}else {
			display_notify('Error','Por favor ingrese direccion!');
		}
