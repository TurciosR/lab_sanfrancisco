...
$id_examen_paciente = $_REQUEST["ref"];
$sql_su = _query("SELECT * FROM examen_paciente WHERE hasha ='$id_examen_paciente'");
$row_su = _fetch_array($sql_su);
$id_examen_paciente = $row_su["id_examen_paciente"];
...
WHERE ep.id_examen_paciente='$id_examen_paciente' AND ep.estado_realizado='Hecho' AND ep.id_sucursal='$id_sucursal' ");
...
$sqlcobro1=_query("SELECT e.id_examen,e.nombre_examen,ep.resultados,mu.muestra,cat.nombre_categoria
  FROM examen_paciente as ep
  JOIN examen as e ON e.id_examen=ep.id_examen
  LEFT JOIN examen_perfil epe ON epe.id_examen=e.id_examen
  LEFT JOIN perfil as pe ON pe.id_perfil=epe.id_perfil
  ...
  WHERE ep.id_examen_paciente='$id_examen_paciente' AND ep.estado_realizado='Hecho' GROUP BY ep.id_examen_paciente");
...
while($rowcobro1 = _fetch_array($sqlcobro1)){
    ...
    $n_categoria = $rowcobro1["nombre_categoria"];
    ...
    if($conteoparametros==1){
        ...
        for($i=0; $i<(count($formulario)-1); $i++){
            ...
            $pdf->Cell(50,3,utf8_decode($campos_valores[1]),0,1,'L');
