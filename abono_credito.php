...
function initial()
{
  ...
  $sql0="SELECT credito.fecha,credito.numero_doc,credito.total,credito.abono,credito.saldo, credito.tipo_doc FROM credito LEFT JOIN paciente ON paciente.id_paciente=credito.id_cliente WHERE credito.id_credito=$id_credito";
  $result = _query($sql0);
    ...
    <?php if ($links!='NOT' || $admin=='1') {
      ...
      <div class="row">
        <div class="form-group col-md-6">
          ...
          <select class="form-control select" id="tipo_pago" style="width:100%;">
        ...
        <div class="form-group col-md-6">
          <label class="a" hidden >Numero Doc.</label>
          <input type="hidden" hidden  class='a form-control input_header_panel' id="num_doc_pago">
        </div>
...
$(document).on('change', '#tipo_pago', function(event) {
  ...
  if (a=="Efectivo") {
    $(".a").prop('hidden', 'true');
    $("#num_doc_pago").attr('type', 'hidden');
    $("#cuenta_banco").prop('hidden', 'true');
  }
  else {
    if(a=="Transferencia")
    {
      $(".a").prop('hidden', '');
      $("#cuenta_banco").prop('hidden', '');
      $("#num_doc_pago").attr('type', 'text');
    }
    else {
      $(".a").prop('hidden', '');
      $("#cuenta_banco").prop('hidden', 'true');
...
<?php
  ...
  {
    echo "<br><br><div class='alert alert-warning' style='font-size:15px;'>No posee una apertura vigente o ya existe una creada por otro usuario</div></div>";
...
function abonar()
{
  ...
  $num_doc_pago = $_POST["num_doc_pago"];
  $tipo = $_POST["tipo"];
  $fecha=date("Y-m-d");
  $hora=date("H:i:s");
