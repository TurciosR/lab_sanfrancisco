                                ...
                                <div class="form-group has-info single-line">
                                    <label>Empleado <span style="color:red;">*</span></label>
                                        <?php
                                            if($admin != 1)
                                            {
                                              $sql_empleado = _query("SELECT * FROM usuario WHERE id_usuario = '$id_user'");
                                            ...
                                            {
                                              $sql_usuario = _query("SELECT id_usuario,id_empleado,nombre,admin FROM usuario WHERE id_empleado>0 or admin = 1 and id_sucursal='$id_sucursal'");

                                              echo "<select class='form-control select' id='empleado' name='empleado'>";
                                              while ($row_usuario = _fetch_array($sql_usuario))
                                              {
                                                $id_usuario = $row_usuario["id_usuario"];
                                                $id_empleado = $row_usuario["id_empleado"];
                                                $admin = $row_usuario["admin"];
                                                $nombre_usu = $row_usuario["nombre"];
                                                if($admin==1){
                                                  $nombre = $row_usuario["nombre"];
                                                  echo "<option value='".$id_usuario."'>".$nombre_usu."</option>";

                                                }
                                                $sql_empleado = _query("SELECT * FROM empleado WHERE id_tipo_empleado = 1 AND id_empleado='$id_empleado' and id_sucursal='$id_sucursal'");
                                                $cuen = _num_rows($sql_empleado);
                                                if($cuen>0 ){
                                                  $nombre = $row_usuario["nombre"];
                                                  echo "<option value='".$id_usuario."'>".$nombre_usu."</option>";
