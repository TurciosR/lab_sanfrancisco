...
function print_ticket($id_factura){
	...
	if($nrows_fact>0){
		...
		$info_factura.=$esp_init."COBRO #".$numero_doc."|";
		...
		//	$id_prod_serv =$row_fact_det['id_prod_serv'];
			...
			$cantidad=$cantidad;
			//linea a linea
			$descrip=utf8_decode(texto_espacios($descripcion,35));
			...
			$subtotal=sprintf("%.2f",$precio_unit);
	...
	$desc = round($total_value_fin * round($porcentaje/100,2),2);
	$tt_fin = $total_value_fin - $desc;
	$info_factura.="|A PAGAR   $ ".sprintf("%.2f",$tt_fin)."\n";
	$total_final_format=sprintf("%.2f",$tt_fin);
...
function print_corte($id_corte){
	...
	$a=str_pad($esp_init."".$desc_tipo,38," ",STR_PAD_RIGHT);
	...
	if($tipo=="X" || $tipo=="Z"){
		//listar devoluciones
		$sql_dev="SELECT id_dev, id_corte, n_devolucion, t_devolucion FROM devoluciones_corte WHERE id_corte='$id_corte'";
		...
		$subtotal=sprintf('%.2f', $subtotal);
		$totalcaja=sprintf('%.2f', $totalcaja);
		$info_factura.="\n".$esp_init1."   EXEN.     GRAV.    TOTAL"."\n";
		$info_factura.=$line1;
		$n=5;
		$sp1=len_num($texento,$n);
		$sp2=len_num($totalt,$n);
		$sp3=len_num($totalt,$n);
		$info_factura.=$esp_init0." TIQUETES: ".$sp1.$texento."".$sp2.$totalt."".$sp3.$totalt."\n";
		$sp1=len_num($fexento,$n);
		$sp2=len_num($totalf,$n);
		$sp3=len_num($totalf,$n);
		$info_factura.=$esp_init0."FACTURAS: ".$sp1.$fexento."".$sp2.$totalf."".$sp3.$totalf."\n";
		$sp1=len_num($cfexento,$n);
		$sp2=len_num($totalcf,$n);
		$sp3=len_num($totalcf,$n);
		$info_factura.=$esp_init0."FISCALES: ".$sp1.$cfexento."".$sp2.$totalcf."".$sp3.$totalcf."\n";
		$info_factura.=$line1;
		$sp1=len_num($tot_exent,$n);
		$sp2=len_num($tot_grav,$n);
		$sp3=len_num($tot_fin,$n);

		$info_factura.=$esp_init0." TOTAL $ : ".$sp1.$tot_exent.$sp3.$tot_fin.$sp3.$tot_fin."\n";
		$info_factura.="\n";
		...
		$d1=str_pad($tinicio,9," ",STR_PAD_LEFT);
		$e1=str_pad($finicio,9," ",STR_PAD_LEFT);
		$f1=str_pad($cfinicio,9," ",STR_PAD_LEFT);
		$w1=str_pad($tfinal,9," ",STR_PAD_LEFT);
		$x1=str_pad($ffinal,9," ",STR_PAD_LEFT);
		$y1=str_pad($cffinal,9," ",STR_PAD_LEFT);
		$m=str_pad($totalnot,9," ",STR_PAD_LEFT);
		$n=str_pad($totalnof,9," ",STR_PAD_LEFT);
		$o=str_pad($totalnocf,9," ",STR_PAD_LEFT);
		$info_factura.=$esp_init0." TIQUETES: ".$d1.$w1.$m."\n";
		$info_factura.=$esp_init0."FACTURAS: ".$e1.$x1.$n."\n";
		$info_factura.=$esp_init0."FISCALES: ".$f1.$y1.$o."\n";
		$info_factura.=$line1;
		$sp1=len_num($total_docs,26);
...
function datos_fact_det($id_factura){
	$sql_fact_det="SELECT examen.nombre_examen,
		detalle_cobro.*
		FROM detalle_cobro
		JOIN examen ON detalle_cobro.id_examen=examen.id_examen
		WHERE  detalle_cobro.id_cobro='$id_factura' AND detalle_cobro.tipo='E'
		UNION ALL
		SELECT  perfil.nombre_perfil as nombre_examen,
			detalle_cobro.*
			FROM detalle_cobro
			JOIN perfil ON detalle_cobro.id_examen=perfil.id_perfil
			WHERE  detalle_cobro.id_cobro='$id_factura' AND detalle_cobro.tipo='P'
