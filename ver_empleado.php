...
function initial(){
	...
	$sql="SELECT em.*, te.descripcion FROM empleado as em, tipo_empleado as te WHERE te.id_tipo_empleado=em.id_tipo_empleado and em.id_empleado='$id_empleado'and em.id_sucursal='$id_sucursal'";
	$result = _query( $sql );
							...
							<?php
								if ($count > 0) {
									for($i = 0; $i < $count; $i ++) {
										...
										echo "<tr><td>Fecha nacimiento</td><td>".$row['fecha_nacimiento']."</td>";
										echo "<tr><td>Cargo</td><td>".$row['descripcion']."</td>";
										echo "</tr>";
